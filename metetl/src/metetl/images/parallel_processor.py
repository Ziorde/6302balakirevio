...
logger = logging.getLogger("metetl.images.parallel_processor")
...
class ParallelImageProcessor:
    ...
    def __init__(self, output_dir: str = str(OUTPUT_DIR), max_workers: int = MAX_WORKERS):
        ...
        logger.info(
            f"ParallelImageProcessor инициализирован. "
            f"Output: {output_dir}, workers: {max_workers}"
        )
    ...
    def process_single_image(
        ...
        Args:
            data: Кортеж (индекс, object_id, изображение, метаданные_как_словарь,
        ...
        pid = os.getpid()
        print(f"[PID {pid}] Обработка '{method_name}' для изображения {index} началась")
        ...
        if method_name == 'convolution':
            ...
            print(f"[PID {pid}] Неизвестный метод '{method_name}', используется гамма-коррекция")
        ...
        print(f"[PID {pid}] Обработка '{method_name}' для изображения {index} завершена")
    ...
    def process_images(
        ...
        for index, object_id, image, metadata in images_data:
            ...
            process_data.append((
                index, object_id, image, metadata_dict,
                self.output_dir, method_name
            ))

        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            results = list(executor.map(
                ParallelImageProcessor.process_single_image,
                process_data
            ))
        ...
        for index, object_id, path in results:
            logger.info(f"  {index}. ID: {object_id} -> {os.path.basename(path)}")
